    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'name' => [
                ...
                'string',
                'max:255',
                Rule::unique('subcounties', 'subcounty_name')->where(fn ($query) => $query->where('county_id', $request->input('county_id'))),
            ],
    ...
    public function update(Request $request, Subcounty $subcounty)
    {
        ...
        $validated = $request->validate([
            'name' => [
                ...
                'string',
                'max:255',
                Rule::unique('subcounties', 'subcounty_name')->ignore($subcounty->id)->where(fn ($query) => $query->where('county_id', $countyId)),
            ],
