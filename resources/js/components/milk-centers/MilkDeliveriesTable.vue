<template>
    <div class="rounded-lg bg-white p-6 shadow space-y-4">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-lg font-semibold text-surface-900">Milk Deliveries</h2>
                <p class="text-sm text-surface-500">All recorded deliveries for this center.</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-surface-200">
                ...
                <tbody class="divide-y divide-surface-200 text-sm text-surface-700">
                    ...
                    <tr v-for="delivery in deliveries" :key="delivery.id" class="hover:bg-surface-50">
                        <td class="px-6 py-4">{{ formatDate(delivery.delivery_date) }}</td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-semibold text-surface-900"><router-link :to="`/admin/farmers/${delivery.farmer.id}`" class="hover:text-primary-600 hover:underline">{{ delivery.farmer.name }}</router-link></div>
                            <div class="text-xs text-surface-400 uppercase tracking-wide"> ID: {{ delivery.farmer.farmer_id }}</div>
                        </td>
