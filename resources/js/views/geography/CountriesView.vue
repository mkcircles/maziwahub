<template>
    <div class="space-y-10 pb-16">
        ...
        <div v-if="countriesStore.loading" class="rounded-md border border-surface-200 bg-white p-8 text-surface-600 shadow-sm border border-surface-200">
            Loading countries…
        </div>

        <div v-else-if="countriesStore.error" class="rounded-md border border-red-200 bg-red-50/80 p-6 text-red-700 shadow-lg shadow-red-100">
            {{ countriesStore.error }}
        </div>

        <div v-else class="space-y-8">
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Countries</p>
                    <p class="mt-2 text-2xl font-semibold text-surface-900">{{ countries.length }}</p>
                    <p class="text-xs text-surface-500">Total registered</p>
                </div>
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Regions</p>
                    <p class="mt-2 text-2xl font-semibold text-surface-900">{{ totalRegionsCount }}</p>
                    <p class="text-xs text-surface-500">Across all countries</p>
                </div>
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Milk Centers</p>
                    <p class="mt-2 text-2xl font-semibold text-surface-900">{{ totalMilkCentersCount }}</p>
                    <p class="text-xs text-surface-500">Supporting dairy infrastructure</p>
                </div>
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Farmers</p>
                    <p class="mt-2 text-2xl font-semibold text-surface-900">{{ totalFarmersCount }}</p>
                    <p class="text-xs text-surface-500">Registered across these countries</p>
                </div>
            </div>

            <div class="grid gap-4 lg:grid-cols-3">
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Average Farmers / Country</p>
                    <p class="mt-2 text-xl font-semibold text-surface-900">{{ averageFarmersPerCountry }}</p>
                    <p class="text-xs text-surface-500">Using the full dataset</p>
                </div>
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Average Milk Centers / Country</p>
                    <p class="mt-2 text-xl font-semibold text-surface-900">{{ averageCentersPerCountry }}</p>
                    <p class="text-xs text-surface-500">Infrastructure coverage</p>
                </div>
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur space-y-2">
                    ...
                    <p class="text-sm text-surface-500">
                        <span class="font-semibold text-surface-900">{{ topCountryByFarmers?.name ?? '—' }}</span>
                        has the most farmers.
                    </p>
                        <p class="text-sm text-surface-500">
                        <span class="font-semibold text-surface-900">{{ topCountryByMilkCenters?.name ?? '—' }}</span>
            ...
            <div class="rounded-md border border-surface-100 bg-white/95 p-8 shadow-sm border border-surface-200 backdrop-blur space-y-6">
                <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                    ...
                    <button
                        class="inline-flex items-center gap-2 rounded-full border border-surface-200 bg-surface-50 px-4 py-2 text-sm font-medium text-surface-600 transition hover:bg-surface-100 hover:text-surface-800 disabled:opacity-50"
                        @click="resetFilters"
                ...
                <div class="grid gap-4 md:grid-cols-4">
                    <div class="md:col-span-2">
                        <label class="text-xs font-semibold uppercase tracking-wide text-surface-500" for="country-search">
                            Search
                        </label>
                    ...
                    <div>
                        <label class="text-xs font-semibold uppercase tracking-wide text-surface-500" for="country-iso-filter">
                            ISO Code
                        </label>
                        <select
                            id="country-iso-filter"
                            v-model="isoFilter"
                            class="mt-2 w-full rounded-lg border border-surface-200 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200"
                        >
                    ...
                    <div>
                        <label class="text-xs font-semibold uppercase tracking-wide text-surface-500" for="country-sort">
                            Sort by
                        </label>
                        <select
                            id="country-sort"
                ...
                <div class="flex flex-wrap items-center gap-3 text-sm text-surface-600">
                    ...
                    <span class="inline-flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-surface-400">
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur space-y-2">
                    ...
                    <p class="text-sm text-surface-500">
                        <span class="font-semibold text-surface-900">{{ filteredTotals.totalCountries }}</span>
                ...
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Average Farmers (Filtered)</p>
                    <p class="mt-2 text-xl font-semibold text-surface-900">{{ filteredTotals.averageFarmers }}</p>
                    <p class="text-xs text-surface-500">Per country in the current results</p>
                </div>
                <div class="rounded-lg border border-white/70 bg-white/90 p-5 shadow-sm border border-surface-200 backdrop-blur">
                    <p class="text-xs font-semibold uppercase tracking-wide text-surface-500">Focus Country</p>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead class="bg-surface-50/70">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-surface-500">
                                    Country
                                </th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-surface-500">
                                    ISO Code
                                ...
                                <th scope="col" class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wide text-surface-500">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 bg-white text-sm text-surface-700">
                            ...
                            <tr
                                ...
                                :key="country.id"
                                class="transition hover:bg-surface-50/60"
                            >
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-surface-900">{{ country.name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="inline-flex items-center rounded-full bg-surface-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-surface-600">
                                        {{ country.iso_code }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <router-link
                                        ...
                                        class="inline-flex items-center gap-1 rounded-full bg-sky-50 px-3 py-1 text-sm font-medium text-primary-700 transition hover:bg-sky-100 hover:text-sky-700"
                                    >
